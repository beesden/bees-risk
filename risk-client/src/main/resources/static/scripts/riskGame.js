...
risk.game = (function (d) {
    ...
    return {
    	...
    	attack: function(data) {
    		...
    		// Show popup and update players on action
    	...
    	gameLobby: function(data) {
    		...
    		for (id in data.gameList) {
    			gameSelectWrap =  utils.createElement('dd');
    			gameSelect = utils.createElement('button', {'value':data.gameList[id]}, gameSelectWrap);
				gameSelect.innerHTML = data.gameList[id];
    	...
    	gameSetup: function(data) {
			...
			for (id in data.playerList) {
				player = utils.createElement('li');
				player.style.borderColor = data.playerList[id];
				player.innerHTML = id;
    	...
    	redeploy: function(fromTerr, toTerr, force, min) {
    		...
    		var maxDeploy = fromTerr.battalions - 1,
    			...
    			redeployRange = utils.createElement('input', {'type':'range', 'max':maxDeploy, 'min':min}, redeployForm),
	    ...
	    viewCards: function(data, callback) {
	        ...
	        for (id in cardArray) {
	            ...
	            cardInput = utils.createElement('input', {'name': card, 'id': card, 'type': 'checkbox', 'value': cardData ? cardData.cardValue : 0}, cardList);
